...
class Program
{
    static void Main()
    {
        ...
        while (running)
        {
            ...
            Console.WriteLine("6. Count teachers per department");
            Console.WriteLine("7. Show student grades");
            Console.WriteLine("8. Show all courses");
            Console.WriteLine("9. Add grade to student");
            ...
            switch (Console.ReadLine())
            {
                ...
                case "6":
                    ...
                    break;
                case "7":
                    ShowStudentGrades(context);
                    break;
                case "8":
                    ShowAllCourses(context);
                    break;
                case "9":
                    AddGradeWithTransaction(context);
                    break;
                case "0":
                    running = false;
                    break;
        ...
        static void CountTeachersPerDepartment(SchoolTestDbContext context)
        {
            var teacherCounts = context.SchoolEmployees
                .Where(e => context.Teachers.Any(t => t.TeacherId == e.EmployeeId))
                .GroupBy(e => e.Role)
                .Select(g => new { Department = g.Key, Count = g.Count() })
                .ToList();

            Console.WriteLine("\nTeachers per department:");
            foreach (var t in teacherCounts)
                Console.WriteLine($"{t.Department}: {t.Count}");
        }


        static void ShowStudentGrades(SchoolTestDbContext context)
        {
            var students = context.Students
                .Select(s => new
                {
            ...
            foreach (var s in students)
            {
                ...
                { 
                        Console.WriteLine($"  {g.CourseName}: {g.GradeValue} (Teacher: {g.TeacherName}, Date: {g.GradeDate.ToShortDateString()})");
                }
            }
        }


        static void ShowAllCourses(SchoolTestDbContext context)
        {
        ...
        static void AddGradeWithTransaction(SchoolTestDbContext context)
        {
